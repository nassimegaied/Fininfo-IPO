...
type CorporateTabContentProps = {
  value: CorporateTabData;
  onChange: (nextValue: CorporateTabData) => void;
};

function createDuplicateItem(
  source: CorporatePrerequisItem,
  id: string
): CorporatePrerequisItem {
  return {
    ...source,
...
export default function CorporateTabContent({
  value,
  onChange,
}: CorporateTabContentProps) {
  const updateItem = (
    itemId: string,
    updater: (current: CorporatePrerequisItem) => CorporatePrerequisItem
  ) => {
    onChange({
      ...value,
  ...
  const duplicateItem = (item: CorporatePrerequisItem) => {
    const nextIndex = value.prerequisItems.length + 1;
    ...
    onChange({
      ...
      prerequisItems: [
        ...value.prerequisItems,
        createDuplicateItem(item, `corp-${nextIndex}`),
      ],
    });
  };
  ...
  const updateParametreField = (
    field: keyof CorporateTabData["parametres"],
    fieldValue: string
  ) => {
    onChange({
      ...value,
  ...
  const updateFraisField = (
    field: keyof CorporateTabData["frais"],
    fieldValue: string
  ) => {
    onChange({
      ...value,
  ...
  const updateMethodePaiementField = (
    field: keyof CorporateTabData["methodePaiement"],
    fieldValue: string
  ) => {
    onChange({
      ...value,
  ...
  const updateAllocationField = (
    field: keyof CorporateTabData["allocation"],
    fieldValue: boolean
  ) => {
    onChange({
      ...value,
  ...
  const updateCouvertureField = (
    field: keyof CorporateTabData["couvertureSouscription"],
    fieldValue: string
  ) => {
    onChange({
      ...value,
        ...
        >
          ...
          >
            ...
            >
              ...
              <ToggleChoice
                text="Salarié"
                active={item.dirigeantSalarie === "salarie"}
                onClick={() =>
                  updateItem(item.id, (current) => ({
                    ...current,
                    dirigeantSalarie:
                      current.dirigeantSalarie === "salarie" ? "" : "salarie",
                  }))
                }
              />
              <ToggleChoice
                text="Dirigeant"
                active={item.dirigeantSalarie === "dirigeant"}
                onClick={() =>
                  updateItem(item.id, (current) => ({
                    ...current,
                    dirigeantSalarie:
                      current.dirigeantSalarie === "dirigeant" ? "" : "dirigeant",
                  }))
                }
              />
            </Box>
          ...
          >
            ...
            >
              <Typography sx={{ fontSize: 13, fontWeight: 700, color: "#1cb7c8" }}>
                Régime de rémunération mensuel :
              </Typography>
              <ToggleChoice
                text="CDI"
                active={item.regimeRemuneration === "cdi"}
                onClick={() =>
                  updateItem(item.id, (current) => ({
                    ...current,
                    regimeRemuneration:
                      current.regimeRemuneration === "cdi" ? "" : "cdi",
                  }))
                }
              />
              <ToggleChoice
                text="CDD"
                active={item.regimeRemuneration === "cdd"}
                onClick={() =>
                  updateItem(item.id, (current) => ({
                    ...current,
                    regimeRemuneration:
                      current.regimeRemuneration === "cdd" ? "" : "cdd",
                  }))
                }
              />
              <ToggleChoice
                text="Autres"
                active={item.regimeRemuneration === "autres"}
                onClick={() =>
                  updateItem(item.id, (current) => ({
                    ...current,
                    regimeRemuneration:
                      current.regimeRemuneration === "autres" ? "" : "autres",
                  }))
                }
              />
            </Box>
            ...
            >
              <ToggleChoice
                text="Titulaire"
                active={item.titulaire}
                onClick={() =>
                  updateItem(item.id, (current) => ({
                    ...current,
                    titulaire: !current.titulaire,
                  }))
                }
              />
              <ToggleChoice
                text="Non démissionnaire"
                active={item.nonDemissionnaire}
                onClick={() =>
                  updateItem(item.id, (current) => ({
                    ...current,
                    nonDemissionnaire: !current.nonDemissionnaire,
                  }))
                }
              />
            </Box>
